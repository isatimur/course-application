...
class AuthorGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Author entity")
        ...
        .repeat(2) {
            ...
            .get("/courseapplication/api/authors")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new author")
            .post("/courseapplication/api/authors")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "description":"SAMPLE_TEXT", "photoUrl":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_author_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created author")
                .get("/courseapplication${new_author_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created author")
            .delete("/courseapplication${new_author_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
